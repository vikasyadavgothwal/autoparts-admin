...
export function LiveOrdersPage({ initialOrders, initialPagination, initialSummary }: {
  ...
  return <div className="space-y-8">
    ...
    <form className="flex max-w-2xl gap-2" onSubmit={(event) => { event.preventDefault(); void load(1) }}><div className="relative flex-1"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#9CA3AF]" /><Input value={search} maxLength={120} onChange={(event) => setSearch(event.target.value.slice(0, 120))} placeholder="Search order ID, RFQ, buyer, supplier, or part..." className="border-[#2A2A2A] bg-[#1A1A1A] pl-9 text-white" /></div><Button type="submit" disabled={loading}>Search</Button>{search ? <Button type="button" variant="outline" onClick={() => { setSearch(""); void load(1, "") }}>Clear</Button> : null}</form>
