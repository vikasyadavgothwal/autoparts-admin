...
import { normalizeVendorSku } from "@/services/parts-mapping/internal-helpers"
...
type CatalogBulkLimitRowError = Pick<
  SupplierBulkProductRow,
  "vendorSku" | "oemNumber" | "brand" | "competitorPartNumber" | "competitorBrandName"
> & {
  rowNumber: number
  reason: string
}

const normalizeCatalogValue = (value: string | null | undefined) => {
  const text = value?.trim().toLowerCase()
  return text ? text : null
}

const getSupplierBusinessPlanState = async (supplierId: string) => {
...
const getCurrentSupplierCatalogValues = async (supplierId: string) => {
  const [brands, categories, existingSkus] = await Promise.all([
    db.supplierPart.findMany({
      where: { supplierId, originalBrand: { not: null } },
      select: { originalBrand: true },
      distinct: ["originalBrand"],
    }),
    db.supplierPart.findMany({
      where: { supplierId, category: { not: null } },
  ...
  return {
    brandSet: new Set(
      brands
        .map((row) => normalizeCatalogValue(row.originalBrand))
        .filter((value): value is string => Boolean(value)),
    ),
    categorySet: new Set(
      categories
        .map((row) => normalizeCatalogValue(row.category))
        .filter((value): value is string => Boolean(value)),
    ),
    existingSkuSet: new Set(
      existingSkus.map((row) => normalizeVendorSku(row.vendorSku)),
    ),
  }
}

const splitRowsBySupplierCatalogLimits = async (
  supplierId: string,
  rows: SupplierBulkProductRow[],
) => {
  const plan = await getSupplierBusinessPlanState(supplierId)
  const catalogValues = await getCurrentSupplierCatalogValues(supplierId)
  const acceptedRows: SupplierBulkProductRow[] = []
  const rejectedRows: CatalogBulkLimitRowError[] = []

  let acceptedNewProductCount = 0

  for (const row of rows) {
    const vendorSku = normalizeVendorSku(row.vendorSku)
    const isNewProduct = vendorSku ? !catalogValues.existingSkuSet.has(vendorSku) : false
    const reasons: string[] = []

    if (isNewProduct && plan.planProductLimit !== null) {
      if (acceptedNewProductCount >= plan.planProductLimit) {
        reasons.push("Product limit reached. Upgrade your plan.")
      }
    }

    const normalizedBrand = normalizeCatalogValue(row.brand)
    if (
      normalizedBrand &&
      !catalogValues.brandSet.has(normalizedBrand) &&
      plan.planBrandLimit !== null &&
      catalogValues.brandSet.size >= plan.planBrandLimit
    ) {
      reasons.push("Brand limit reached. Upgrade your plan.")
    }

    const normalizedCategory = normalizeCatalogValue(row.category)
    if (
      normalizedCategory &&
      !catalogValues.categorySet.has(normalizedCategory) &&
      plan.planCategoryLimit !== null &&
      catalogValues.categorySet.size >= plan.planCategoryLimit
    ) {
      reasons.push("Category limit reached. Upgrade your plan.")
    }
    ...
    if (isNewProduct) acceptedNewProductCount += 1

    if (normalizedBrand) catalogValues.brandSet.add(normalizedBrand)
    if (normalizedCategory) catalogValues.categorySet.add(normalizedCategory)
  }

  return { accountId: plan.accountId, acceptedRows, rejectedRows }
}
  ...
  try {
    ...
    if (workbookFile) {
      ...
      await logBusinessActivity({
        ...
        action: "supplier_products.bulk_uploaded",
      ...
      return NextResponse.json({
        ...
        summary: {
          ...summary,
          ...{
            totalRows: summary.totalRows + rejectedRows.length,
            unmappedCount: combinedUnmapped.length,
            unmapped: combinedUnmapped,
          },
          lookupSummary,
    ...
    await logBusinessActivity({
      ...
      action: "supplier_products.bulk_uploaded",
      entityType: "supplier_part",
      metadata: { rows: acceptedRows.length },
    })
    const acceptedProductSkus = new Set(
      acceptedRows.map((row) => normalizeVendorSku(row.vendorSku)),
    )
    const unmapped = [...summary.unmapped, ...rejectedRows]
    const unmatchedImageRows = imageRows
      .filter((row) =>
        !acceptedProductSkus.has(normalizeVendorSku(row.vendorSku)),
      )
      .map((row) => ({
        rowNumber: row.rowNumber,
