...
import { buildSupplierProductMasterCsvExport } from "@/services/parts-mapping/export.service"
...
export async function GET(request: NextRequest) {
  ...
  try {
    const exportData = await buildSupplierProductMasterCsvExport(auth.user.id)
    ...
    return NextResponse.json(
      {
        ok: false,
        message: error instanceof Error ? error.message : "Unable to export product CSV",
      },
      { status: 500 },
    )
  }
