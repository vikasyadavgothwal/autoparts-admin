...
import { apiError, apiErrorMessage, apiOk, withSupplierApiRoute } from "@/lib/auth/api-guards"
...
export async function GET(request: NextRequest) {
  return withSupplierApiRoute(request, async (user) => {
    try {
      await assertBusinessMenuAccess({ userId: user.id, accountType: BusinessAccountType.Supplier, menuKey: "offers" })
    } catch (error) {
      return apiError(apiErrorMessage(error, "You do not have permission to view offers"), 403)
    }
